...
public void insertFood(FoodEntity f) 
{
   ...
   database.insert("FoodDetails", null, newFood);
...
{
	Cursor result = database.query("FoodDetails", null, 
...
{
	Cursor result = database.rawQuery("SELECT * FROM FoodDetails WHERE name = '"+name.trim()+"' COLLATE NOCASE", null); 
	int nameIndex;
	int calIndex, barindex;
	FoodEntity e1 = null;
	if( result != null && result.moveToFirst() )
...
{
	Cursor result = database.rawQuery("SELECT * FROM FoodDetails WHERE barcode = '"+bcode.trim()+"' COLLATE NOCASE", null); 
	int nameIndex;
	int calIndex, barindex;
	FoodEntity e1 = null;
	if( result != null && result.moveToFirst() )
